...
def _transcribe_dual_channel(audio_file: Path, config: TranscriptionConfig, duration: float) -> Transcript:
    ...
    try:
        ...
        # Pre-compute padding
        pad_samples = int(config.audio_pad_seconds * 16000) if config.audio_pad_seconds > 0 else 0

        # ── Transcribe mic channel ──
        print(f"  Transcribing mic channel (left)...")
        mic_audio = whisperx.load_audio(str(mic_path))
        if pad_samples > 0:
            mic_audio = np.concatenate([mic_audio, np.zeros(pad_samples, dtype=mic_audio.dtype)])
        ...
        if pad_samples > 0:
            sys_audio = np.concatenate([sys_audio, np.zeros(pad_samples, dtype=sys_audio.dtype)])
        ...
        if config.skip_alignment:
            ...
            try:
                ...
                mic_result = whisperx.align(
                    mic_result["segments"], model_a, metadata,
                    mic_audio, config.device, return_char_alignments=False,
                )
                sys_result = whisperx.align(
                    sys_result["segments"], model_a, metadata,
                    sys_audio, config.device, return_char_alignments=False,
                )
                del model_a
                gc.collect()
                torch.cuda.empty_cache()
